...
PaddingLm1bCharEmbedding::PaddingLm1bCharEmbedding()
{
    ...
    while (std::getline(infile, line)) {
        ...
        std::transform(v.begin(), v.end(), emb.begin(), [](auto& s) { return atof(s.c_str()); });
        embeddings_by_char_.emplace_back(std::move(emb));
    }
}

std::vector<double> PaddingLm1bCharEmbedding::get_embedding(const std::string& word) const
{
...
void PaddingLm1bCharEmbedding::add_emb(char c, std::vector<double>& embs) const
{
    std::copy(embeddings_by_char_[c].begin(), embeddings_by_char_[c].end(), std::back_inserter(embs));
}
...
std::vector<double> SquashedLm1bCharEmbedding::get_embedding(const std::string& word) const
{
