...
Tree::Tree(int leaf_count, const vector<int> &children, int arity)
    : leaf_count_(leaf_count), arity_(arity), root_node_(-1) // to be filled
      ...
      ,
      children_(children), path_lengths_(leaf_count),
  ...
  if ((leaf_count - 1) % (arity_ - 1) != 0) {
    fprintf(stderr, "Cannot build a full tree of arity %d from %d leaf nodes\n",
            arity_, leaf_count);
    exit(1);
  }
  ...
  for (int leaf_node = 0; leaf_node < leaf_count; leaf_node++) {
    ...
    for (int node = leaf_node; node != root_node_; node = parent_node[node]) {
      if (path_length == MAX_HSTREE_DEPTH) {
        fprintf(stderr,
                "ERROR Cannot build a tree with height greater than %d\n",
                MAX_HSTREE_DEPTH);
        exit(1);
      }
...
HSTree::HSTree(const Vocabulary &vocab, int layer_size,
    ...
    : vocab_(vocab), layer_size(layer_size),
      ...
      weights_(vocab.size(), layer_size + char_embedding.size()),
...
HSTree *HSTree::CreateHuffmanTree(const Vocabulary &vocab, int layer_size,
  ...
  for (int new_node = vocab.size(); new_node < node_count; new_node++) {
    ...
    for (int branch = 0; branch < arity; ++branch) {
      if (next_leaf_node >= 0 &&
          weight[next_leaf_node] < weight[next_inner_node]) {
...
HSTree::~HSTree() { delete tree_; }

void HSTree::Dump(FILE *fo) const { ::Dump(weights_, fo); }

void HSTree::Load(FILE *fo) { ::Load(&weights_, fo); }

// see the comment in the header
int HSTree::DetectEffectiveMaxentOrder(WordIndex target_word,
                                       const MaxEnt *maxent,
                                       const uint64_t *feature_hashes,
                                       int max_maxent_order) const {
...
template <int kArity>
inline void
CalculateNodeChildrenScores(const HSTree *hs, int node, const Real *hidden,
                            const uint64_t *feature_hashes, int maxent_order,
                            const MaxEnt *maxent, Real *branch_scores,
...
// otherwise - dynamic
template <class T, int kStaticSize> class MaybeStaticArray {
public:
...
// feature_hashes must contain at least maxent_order elements
template <int kArity>
inline void PropagateNodeForward(const HSTree *hs, int node, const Real *hidden,
                                 const uint64_t *feature_hashes,
                                 int maxent_order, const MaxEnt *maxent,
                                 double *state, const std::string &curr_chars) {
  const int arity = (kArity == kDynamic) ? hs->tree_->GetArity() : kArity;
  MaybeStaticArray<Real, kArity> tmp(arity);
  CalculateNodeChildrenScores<kArity>(hs, node, hidden, feature_hashes,
                                      maxent_order, maxent, tmp.data(),
                                      curr_chars);
...
template <int kArity>
inline void PropagateNodeForwardByDepth(const HSTree *hs, WordIndex target_word,
                                        const Real *hidden, int depth,
                                        const uint64_t *feature_hashes,
                                        int maxent_order, const MaxEnt *maxent,
                                        double *state) {
  int node = hs->tree_->GetPathToLeaf(target_word)[depth];
  auto word = std::string(hs->vocab_.GetWordByIndex(target_word));
  return PropagateNodeForward<kArity>(hs, node, hidden, feature_hashes,
                                      maxent_order, maxent, state, word);
}
...
// feature_hashes must contain at least maxent_order elements
template <int kArity>
inline void PropagateNodeBackward(HSTree *hs, WordIndex target_word, int depth,
                                  const uint64_t *feature_hashes,
                                  int maxent_order, Real lrate,
                                  Real maxent_lrate, Real l2reg,
                                  Real maxent_l2reg, Real gradient_clipping,
                                  const double *state, const Real *hidden,
                                  Real *hidden_grad, MaxEnt *maxent) {
  // calculate gradients d(ln(sigma(x, selected_branch))) / d(x_i), i <= n - 1
  const int selected_branch =
      hs->tree_->GetBranchPathToLead(target_word)[depth];
  // gradient
  const int arity = (kArity == kDynamic) ? hs->tree_->GetArity() : kArity;
  ...
  for (int branch = 0; branch < arity - 1; ++branch) {
    ...
    int child_offset =
        hs->tree_->GetChildOffsetByDepth(target_word, depth, branch);
    ...
    for (int order = 0; order < maxent_order; ++order) {
      uint64_t maxent_index = feature_hashes[order] + child_offset;
      maxent->UpdateValue(maxent_index, maxent_lrate, maxent_grad,
                          maxent_l2reg);
    }
  }
}
...
Real PropagateForwardAndBackwardReal(
    HSTree *hs, bool calculate_probability, WordIndex target_word,
    const uint64_t *feature_hashes, int maxent_order, Real lrate,
    Real maxent_lrate, Real l2reg, Real maxent_l2reg, Real gradient_clipping,
    const Real *hidden, Real *hidden_grad, MaxEnt *maxent) {
  MaybeStaticArray<double, kArity> softmax_state(hs->tree_->GetArity());
  Real logprob = 0.;
  for (int depth = 0; depth < hs->tree_->GetPathLength(target_word) - 1;
       depth++) {
    PropagateNodeForwardByDepth<kArity>(hs, target_word, hidden, depth,
                                        feature_hashes, maxent_order, maxent,
                                        softmax_state.data());

    PropagateNodeBackward<kArity>(
        hs, target_word, depth, feature_hashes, maxent_order, lrate,
        maxent_lrate, l2reg, maxent_l2reg, gradient_clipping,
        softmax_state.data(), hidden, hidden_grad, maxent);
...
Real HSTree::PropagateForwardAndBackward(
  ...
  if (tree_->GetArity() == 2) {
    return PropagateForwardAndBackwardReal<2>(
        ...
        lrate, maxent_lrate, l2reg, maxent_l2reg, gradient_clipping, hidden,
        hidden_grad, maxent);
  }
...
std::vector<double> HSTree::ChildProbs(int node, const uint64_t *feature_hashes,
                                       int maxent_order,
                                       bool dynamic_maxent_prunning,
                                       const Real *hidden, const MaxEnt *maxent,
                                       const std::string &curr_chars) const {
  ...
  PropagateNodeForward<kDynamic>(this, node, hidden, feature_hashes,
                                 maxent_order, maxent, softmax_state.data(),
                                 curr_chars);
  return softmax_state;
}

// see the comment in the header
Real HSTree::CalculateLog10Probability(WordIndex target_word,
                                       const uint64_t *feature_hashes,
                                       int maxent_order,
                                       bool dynamic_maxent_prunning,
                                       const Real *hidden, const MaxEnt *maxent,
...
void HSTree::SampleWord(const uint64_t *feature_hashes, int maxent_order,
                        const Real *hidden, const MaxEnt *maxent,
                        Real *logprob_, WordIndex *sampled_word_,
                        const std::string &curr_chars) const {
  ...
  while (!tree_->IsLeaf(node)) {
    ...
    PropagateNodeForward<kDynamic>(this, node, hidden, feature_hashes,
                                   maxent_order, maxent, probabilities.data(),
                                   curr_chars);
