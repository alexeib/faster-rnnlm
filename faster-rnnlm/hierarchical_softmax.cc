...
HSTree::DiverseCandidates(int node, int curr_depth, int target_depth, const uint64_t *feature_hashes, int maxent_order,
    ...
    if (curr_depth < target_depth) {
        ...
        auto max_branch = std::distance(child_probs.begin(), std::max_element(child_probs.begin(), child_probs.end()));
...
HSTree::DiverseCandidates(int target_number, const uint64_t *feature_hashes, int maxent_order,
    ...
    DiverseCandidates(tree_->GetRootNode(), 0, target_depth, feature_hashes, maxent_order, dynamic_maxent_prunning,
                      hidden,
                      maxent);
