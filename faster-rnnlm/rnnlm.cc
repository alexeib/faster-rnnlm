...
Real EvaluateLM(NNet *nnet, const std::string &filename, bool print_logprobs,
  ...
  while (reader.Read()) {
    ...
    if (!nnet->cfg.use_nce) {
      ...
      for (int target = 1; target <= seq_length; ++target) {
        ...
        if (std::isnan(logprob) || std::isinf(logprob)) {
          printf("logprob is nan/inf for word %s\n", word.c_str());
        }
    ...
    if (std::isnan(sen_logprob) || std::isinf(sen_logprob)) {
      printf("sen_logprob is nan\n");
    }
    if (std::isnan(seq_length) || std::isinf(seq_length)) {
      printf("seq_length is nan\n");
    }
  ...
  printf("logprob_sum: %f, n_words: %d", logprob_sum, n_words);
...
int main(int argc, char **argv) {
  ...
  std::string model_vocab_file, test_file, train_file, valid_file,
      complete_phrase, replacement_candidates;
  ...
  opts.Add(
      ...
      "Word replacement candidates, format [phrase]:[word index to replace]",
  ...
  if (test_file.empty() && train_file.empty() && n_samples == 0 &&
      complete_phrase.empty() && replacement_candidates.empty()) {
