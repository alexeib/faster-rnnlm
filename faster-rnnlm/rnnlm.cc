...
void TopCandidates(MixtureNet& mn, const std::string s)
{
    ...
    std::vector<std::tuple<std::string, Real>> candidatesWithProb;
    auto sen = wids;
    std::string currWord = &input[0][idx];
    for (WordIndex c = 0; c<vocabSize; ++c) {
        sen[idx] = c;
        candidatesWithProb.emplace_back(vocab.GetWordByIndex(c),
                mn.Log10WordProbability(sen, idx, &currWord));
    }

    std::partial_sort(candidatesWithProb.begin(), candidatesWithProb.begin()+10, candidatesWithProb.end(),
            [](auto& a, auto& b) { return std::get<1>(a)>std::get<1>(b); });
...
int main(int argc, char** argv)
{
    ...
    opts.Add(
            "diverse-candidates",
            "Diverse word replacement candidates, format [phrase]:[word index to replace]",
            &diverse_candidates);
    opts.Add(
            "top-candidates",
            "Top word replacement candidates, format [phrase]:[word index to replace]",
            &top_candidates);
    ...
    if (test_file.empty() && train_file.empty() && n_samples==0 &&
            complete_phrase.empty() && diverse_candidates.empty() && top_candidates.empty()) {
