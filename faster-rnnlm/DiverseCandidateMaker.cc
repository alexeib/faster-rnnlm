...
std::vector<WordIndex>
DiverseCandidateMaker::DiverseCandidates(int node,
                                         int curr_depth,
                                         int target_depth,
                                         bool dynamic_maxent_prunning,
                                         int sentence_length,
                                         int word_pos,
                                         const DirData &forward,
                                         const DirData &reverse,
  ...
  auto forward_child_probs = forward.nnet->softmax_layer->ChildProbs(node,
                                                                     forward.feature_hashes,
                                                                     forward.maxent_present,
                                                                     dynamic_maxent_prunning,
                                                                     forward.hidden,
                                                                     &forward.nnet->maxent_layer,
                                                                     curr_word);
  auto reverse_child_probs = reverse.nnet->softmax_layer->ChildProbs(node,
                                                                     reverse.feature_hashes,
                                                                     reverse.maxent_present,
                                                                     dynamic_maxent_prunning,
                                                                     reverse.hidden,
                                                                     &reverse.nnet->maxent_layer,
                                                                     curr_word);
  auto child_probs = combinedProbs(forward_child_probs, reverse_child_probs, sentence_length, word_pos);
  if (curr_depth <= target_depth) {
    auto
        num_above_thresh =
        std::count_if(child_probs.begin(), child_probs.end(), [=](auto p) { return p >= threshold; });
    for (int i = 0; i < tree->GetArity(); ++i) {
      if (child_probs[i] >= threshold) {
        auto v = DiverseCandidates(children[i],
                                   curr_depth + (num_above_thresh > 1 ? 1 : 0),
                                   target_depth,
                                   dynamic_maxent_prunning,
                                   sentence_length,
                                   word_pos,
    ...
    if (tree->IsLeaf(selected_node)) {
      ...
      auto v = DiverseCandidates(selected_node, curr_depth + 1, target_depth, dynamic_maxent_prunning,
                                 sentence_length, word_pos, forward, reverse, curr_word, threshold);
      res.insert(res.end(), v.begin(), v.end());
    }
  }
  return res;
